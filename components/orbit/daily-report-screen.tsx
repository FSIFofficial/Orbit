    ...
    <div className="mx-auto max-w-2xl px-4 py-8">
      <div className="mb-6 flex items-center justify-between">
        <div className="flex items-center gap-2.5">
          ...
          <h1 className="text-xl font-semibold">日報・週報</h1>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
          ...
          <div className="space-y-1.5">
            <label className="block text-sm font-medium">
              {type === 'daily' ? '今日やったこと' : '今週やったこと'} <span className="text-destructive">*</span>
            </label>
            <textarea
              value={done}
              onChange={(e) => setDone(e.target.value)}
              placeholder="完了したタスク・取り組んだこと"
              rows={4}
              required
          ...
          <div className="space-y-1.5">
            <label className="block text-sm font-medium">
              {type === 'daily' ? '明日やること' : '来週やること'}
            </label>
            <textarea
              value={todo}
              onChange={(e) => setTodo(e.target.value)}
              placeholder="次に取り組む予定"
              rows={3}
              className="w-full rounded-lg border border-border bg-background px-3 py-2 text-sm outline-none focus:ring-2 focus:ring-primary/40"
            />
          </div>
          ...
          <div className="space-y-1.5">
            <label className="block text-sm font-medium">課題・困っていること</label>
            <textarea
              value={issues}
              onChange={(e) => setIssues(e.target.value)}
              placeholder="ブロッカー・相談したいこと（任意）"
              rows={2}
              className="w-full rounded-lg border border-border bg-background px-3 py-2 text-sm outline-none focus:ring-2 focus:ring-primary/40"
            />
          </div>
        ...
        <div className="space-y-3">
          {reports.length === 0 ? (
            <div className="flex flex-col items-center justify-center rounded-xl border border-dashed border-border bg-card py-16 text-center">
              ...
              <p className="mt-3 text-sm text-muted-foreground">まだ日報・週報がありません</p>
            </div>
          ) : (
            reports.map((r) => (
              <div key={r.id} className="rounded-xl border border-border bg-card p-4">
                ...
                {r.done && (
                  <div className="mb-2">
                    <p className="text-xs font-medium text-muted-foreground">やったこと</p>
