...
> **重要**: このシートは**「ウェブに公開」しないでください**。`setupOrbit()` を実行するとこのシートが無ければ自動作成されますが、公開設定は行われません（新規作成したシートはデフォルトで非公開です）。読み書きはすべて認証済みのGASアクション（ログイン時の`resolveLogin`、本人のみが呼べる`getMyEmails`/`updateEmail`）経由で行われ、フロントエンドが公開CSV経由でメールアドレスを一括取得することはありません。

> **既存のスプレッドシートを更新する場合**: 以前のバージョンではMembersシートに`email`列があり、そこにメールアドレスが入っています。`Code.gs`を貼り直した後、GASエディタ上部の関数ドロップダウンで **`migrateMemberEmailsToPrivateSheet`** を選び、▶ 実行してください（1回でOK、何度実行しても安全）。既存の`email`列の値をこのMemberEmailsシートへコピーし、コピーを確認できた行だけMembersシート側のemail列を空にします。実行後、実行ログでコピー件数を確認できます。
